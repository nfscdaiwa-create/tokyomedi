...
export function shell(l,req,title,description,active,body,extraLd=''){
 ...
 return `<!doctype html><html lang="${l==='zh-hans'?'zh-Hans':l}"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>${esc(pageTitle(l,title))}</title><meta name="description" content="${esc(description)}"><meta name="theme-color" content="#102630"><link rel="canonical" href="${canonical}">${alts}<meta property="og:type" content="website"><meta property="og:site_name" content="TOKYO MEDI"><meta property="og:title" content="${esc(pageTitle(l,title))}"><meta property="og:description" content="${esc(description)}"><meta property="og:url" content="${canonical}">${jsonLd({'@context':'https://schema.org','@type':'WebSite',name:'TOKYO MEDI',url:SITE,inLanguage:l==='zh-hans'?'zh-Hans':l})}${extraLd}<style>${CSS}</style></head><body>
 <header class="top"><div class="wrap bar"><a class="brand" href="${p(l)}"><span class="brandMark">+</span><span>TOKYO MEDI<small>JAPAN HEALTHCARE REFERENCE</small></span></a><nav class="nav">${nav.map(n=>`<a class="${active===n?'on':''}" href="${p(l,n)}">${esc(t.nav[n])}</a>`).join('')}</nav><div class="headTools"><a class="headSearch" href="${p(l,'medicines')}">⌕ ${esc(t.search)}</a><select class="lang" aria-label="Language" onchange="location.href=this.value">${LOCALES.map(x=>`<option value="${swapLocale(url.pathname,x)}${url.search}" ${x===l?'selected':''}>${localeName[x]}</option>`).join('')}</select></div></div></header>
 ${body}
...
export function securityHeaders(extra={}){return {'x-content-type-options':'nosniff','x-frame-options':'SAMEORIGIN','referrer-policy':'strict-origin-when-cross-origin','permissions-policy':'camera=(), microphone=(), geolocation=()','content-security-policy':"default-src 'self'; style-src 'self' 'unsafe-inline'; script-src 'self' 'unsafe-inline'; img-src 'self' data:; connect-src 'self'; frame-ancestors 'self'; base-uri 'self'; form-action 'self' mailto:",...extra};}
