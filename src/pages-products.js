...
import {esc,p,searchText,shell,jsonLd,breadcrumbLd,htmlLang} from './core.js';
...
export function healthPage(l,req){
 const t=UI[l],u=new URL(req.url),rawQ=(u.searchParams.get('q')||'').trim().slice(0,120),q=searchText(rawQ);
 const items=q?healthProducts.filter(x=>searchText([x.zh,x.ja,x.en,x.brand,x.sku,x.pack,x.kind].join(' ')).includes(q)):healthProducts;
